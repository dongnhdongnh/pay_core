...
namespace Vakapay.Commons.Helpers
{
    public class GoogleAuthen
    {
        public class TwoFactorAuthenticator
        {
            ...
            public SetupCode GenerateSetupCode(string accountTitleNoSpaces, string accountSecretKey, int qrCodeWidth,
                int qrCodeHeight)
            {
            ...
            {
                return GenerateSetupCode(issuer, accountTitleNoSpaces, accountSecretKey, qrCodeWidth, qrCodeHeight,
                    false);
            }
                ...
                int qrCodeWidth, int qrCodeHeight, bool useHttps)
            {
                ...
                {
                    provisionUrl = UrlEncode(String.Format("otpauth://totp/{0}?secret={1}&issuer={2}",
                        accountTitleNoSpaces, encodedSecretKey, UrlEncode(issuer)));
                }

                string protocol = useHttps ? "https" : "http";
                string url = String.Format("{0}://chart.googleapis.com/chart?cht=qr&chs={1}x{2}&chl={3}", protocol,
                    qrCodeWidth, qrCodeHeight, provisionUrl);
