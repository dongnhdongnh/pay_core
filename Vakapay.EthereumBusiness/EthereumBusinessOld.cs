...
namespace Vakapay.EthereumBusiness
{
	...
	public class EthereumBusinessOld : BlockchainBusiness
	{
		...
		public EthereumBusinessOld(IVakapayRepositoryFactory _vakapayRepositoryFactory, bool isNewConnection = true, string endPoint = "") :
		...
		//		List<EthereumWithdrawTransaction> pendings = ethereumwithdrawRepo.FindBySql(SqlHelper.Query_Search(searchDic));
		...
		public ReturnObject CreateNewAddAddress(string walletId)
		{
			try
			{
				...
				var ResultMakeAddress = ethereumRpc.CreateNewAddress(password);
