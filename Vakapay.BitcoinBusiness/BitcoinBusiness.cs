...
namespace Vakapay.BitcoinBusiness
{
    public class BitcoinBusiness : AbsBlockchainBusiness
    {
        ...
        public override List<BlockchainTransaction> GetAllHistory(out int numberData,string userID,string currency,int offset = -1, int limit = -1, string[] orderBy = null)
        {
            ...
            return GetAllHistory<BitcoinWithdrawTransaction,BitcoinDepositTransaction>(out numberData, userID,currency, withdrawRepo, depositRepo,inter.GetTableName(), offset, limit, orderBy);
        }
    }
}