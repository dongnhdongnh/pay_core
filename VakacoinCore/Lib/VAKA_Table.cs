...
{
    public class VAKA_Table<T> where T : IVAKATable
    {
        ...
        List<VAKA_Table<T>> subsets = new List<VAKA_Table<T>>();
        Uri _uri;
        Type t = typeof(T);
        ...
        async Task<VAKA_Table<T>> GetDataSubset(string lower_bound, int limit)
        {
            var rowObjType = (T)Activator.CreateInstance(typeof(T));
            VAKATableMetadata metadata = ((T)Activator.CreateInstance(typeof(T))).GetMetaData();
            ...
            return JsonConvert.DeserializeObject<VAKA_Table<T>>(responseString);
        }
    }
}
